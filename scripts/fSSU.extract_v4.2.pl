...
if($bc ne "none"){
	...
	$h_barcodes{"CTAGTACGATAGAGAG"} = 0; $h_a1_barcodes{"CTAGTACG"} = 0; $h_a2_barcodes{"ATAGAGAG"} = 0; ### Load a set of standard barcodes
}
...
while (my $line1 = <INR1>){                                                                        # The anchors are in R1
	...
	if ($line_nr == 2){
		...
		my $potential_a1 = substr($line1, 19, 30);		                                       # Extract position -3 and +10 of theoretical A1 barcode + Index (22, 18), this is done due to size variation in the index
		my $a1_barcode = "None";
		my $a1_index = 0;
		foreach my $key (keys %h_a1_barcodes){                  
			if ($potential_a1 =~ m/$key/) {                                                     # The barcode should be in the target area
				...
				if ($sl[1] =~ m/ATCTTC/) {	
					...
					next if($a1_length < 9 or $a1_length > 11);                               # Reject if the index length is outside pre-defined criterias			
		...
		my $potential_a2 = substr($line1, 69, 38);		     	                             # Extract position -10 and +10 of theoretical A1 barcode + Index (79, 18)
		my $a2_barcode = "None";
		my $a2_index = 0;
		foreach my $key (keys %h_a2_barcodes){
			if ($potential_a2 =~ m/$key/) {
				...
				if ($sl[0] =~ m/ATCCAT/) {	
					...
					next if($a2_length < 9 or $a2_length > 11);
					$a2_index = $sl2[1];
					$a2_barcode = $key;
 				}
...
#		my $potential_barcode = substr($line2, 10, 30);		     	                        # Extract position -10 and +2 of theoretical A1 barcode (20, 18). This is done to allow indel variation, which is rather common in long synthetic oligo's
...
#				if ($sl[0] =~ m/TGAAGAT/) {	
#					my @sl2 = split(/TGAAGAT/, $sl[0], 2);
					my $a1_length = length($sl[0]);					
					next if($a1_length < 9 or $a1_length > 11);
...
#		my $potential_barcode = substr($line2, 10, 30);		     	                        # Extract position -10 and +2 of theoretical barcode and index (20, 18)
...
#					my @sl2 = split(/TGAAGAT/, $sl[0], 2);
					my $a1_length = length($sl[0]);					
					next if($a1_length < 9 or $a1_length > 11);                               # Remove any index that is outside +/- 1 of the expected index size (10) 
					my $a1_index = substr($sl[0], 0, $a1_length);
					if (exists($h_a1_keep{$a1_index})){ 
...
#					my @sl2 = split(/CATCCAT/, $sl[0], 2);
					my $a2_length = length($sl[0]);					
					next if($a2_length < 9 or $a2_length > 11);                               # Remove any index that is outside +/- 1 of the expected index size (10) 
					my $a2_index = substr($sl[0], 0, $a2_length);					
					if (exists($h_a2_keep{$a2_index})){ 
...
# 				}   
	...
	# For progress reporting
		$progress_count++;
		$progress_total++;
		if ($progress_count >= ($stats_reads_total / 10)){
			$progress_count = 0;
...
fSSU.extract_v4.2.pl  -f -r [-h -a -s -t -q -b -m -p -x]

 [-help -h]           Displays this basic usage information.
 [-R1 -f]             Forward read in fastq format (default: R1.fq).
 [-R2 -r]             Reverse read in fastq format (default: R2.fq).
 [-barcodes -b]       File specifying barcodes to be used (default: 8 standard barcodes).
